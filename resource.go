...
var (
	...
	resourceMapData     []byte
	resourceArchiveData []byte
)
...
func parseMapFile(filename string) *TMapFile {
	b := resourceMapData
	if len(b) == 0 {
		panic("RESOURCE.MAP has not been loaded")
	}
...
func parseResourceFile(filename string, mapFile *TMapFile) {
	b := resourceArchiveData
	if len(b) == 0 {
		panic("RESOURCE.001 has not been loaded")
	}
...
func findBMPResource(name string) *TBMPResource {
	if resource, ok := lookupBMPResource(name); ok {
		return resource
	}
	panic("bmp resource: " + name + " not found")
}

func lookupBMPResource(name string) (*TBMPResource, bool) {
	for i := 0; i < numBmpResources; i++ {
		if bmpResources[i].ResName == name {
			return &bmpResources[i], true
		}
	}
	return nil, false
}
