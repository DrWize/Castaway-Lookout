...
{
    return settings != NULL && settings->sky <= JCENGINE_SKY_NIGHT &&
...
esp_err_t jccontrol_load_settings(jccontrol_settings_t *settings)
{
    ...
    *settings = (jccontrol_settings_t){
    ...
    uint8_t schema = 0, sky = 0, holiday = 0;
    ...
    nvs_close(handle);
    jccontrol_settings_t loaded = {.sky = sky, .holiday = holiday};
    if (valid && schema == SETTINGS_SCHEMA &&
...
{
    static const char *const names[] = {"automatic", "day", "night"};
    return sky <= JCENGINE_SKY_NIGHT ? names[sky] : "unknown";
}
...
bool jccontrol_parse_sky(const char *value, jcengine_sky_mode_t *sky)
{
    ...
    for (int index = 0; index <= JCENGINE_SKY_NIGHT; ++index) {
