...
20. [ ] **Prove panel performance after background batching.** Re-run hashes,
    ...
    every centisecond engine tick and exact rewind behavior. On 2026-09-04 a
    monitor reset of the already-flashed normal candidate passed the PSRAM test,
    resource/hash fixtures, rendered checkpoints, exact rewind and all 63
    load/start checks, then resumed randomized playback and LAN service without
    panic or watchdog. Direct panel smoothness remains the closing evidence.
    The stable sidebar keeps the native scene region unchanged and clears only the reserved
    160x480 sidebar when Off. Its 41-test Python suite, catalog check, uncached
    Go tests, normal build and packaged-flasher gate pass. The release helper
    generated byte-identical canonical data, positively identified the N16R8
    board, verified every write and hard-reset it without erasing NVS. The user
    accepted the default-Off black sidebar; latest Clock/Reviewer switching,
    update-label spacing, smoothness and reboot persistence remain pending.
...
29. [x] **Add the persisted Special Days selector.** Provide Off, Automatic,
...
The stable sidebar and release work does not change scene composition: the 640x480
canvas remains at `x=0..639`, while `x=640..799` is Off, combined Clock/weather,
or Reviewer. Weather/network work stays outside engine ticks. Automated, build,
package-flasher, board-identification, write verification and serial gates pass.
Direct Clock/weather/Reviewer appearance, switching and persistence remain open.
...
Wi-Fi/SNTP, weather, authenticated web controls and the three-state sidebar are
implemented. Touch-menu/layout parity and Soft CRT remain later phases and must
not defer or weaken the all-scenes fidelity gates above.
