...
[Documentation index](README.md) · [Install Castaway Lookout](../README.md)

This reference preserves the technical material moved from the main README on
2026-09-05. Commands run from the repository root unless stated otherwise.
Windows uses Go/Raylib; the independent ESP32 port uses ESP-IDF C. There is no
macOS release of this project.

For ESP32 development, start with the [ESP32 reference](../esp32/README.md) and
[runbook](../esp32/RUNBOOK.md). Current release status and outstanding acceptance
are tracked in the [release plan](RELEASE_READINESS_PLAN.md), [TODO](../TODO.md)
and [ESP32 physical review ledger](../esp32/SCENE_REVIEW.md).
...
The existing Settings panel still displays the former repository URL. GitHub
redirects it to `github.com/DrWize/Castaway-Lookout`; runtime UI and released
Windows binaries were not changed by the documentation rename.
...
> This repository contains no Sierra data files. The Windows installer can
> download a verified external archive; ESP32 and portable users supply original
> files themselves. These files remain Sierra/Dynamix intellectual property.
...
application verifies supported files by hash. The separate Windows installer
can obtain a verified archive through its Data and Sound Setup helper.
...
Historical resource comparison also inspected an upstream Apple Silicon macOS
bundle and recorded matching archive hashes. This is data-comparison evidence,
not a macOS build or installation target of Castaway Lookout.
